...
const ProfilePage = ({ setIsAuthenticated, setIsAdmin }: ProfilePageProps) => {
    ...
    const fetchProfileData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const resAppts = await fetch('https://m1-4.ephec-ti.be:5000/api/appointments', {
            credentials: 'include'
          });
          if (resAppts.ok) {
          ...
          const resOrders = await fetch('https://m1-4.ephec-ti.be:5000/api/orders', {
            credentials: 'include'
          });
          if (resOrders.ok) {
  ...
  const handleSave = async () => {
    try {
      const res = await fetch('https://m1-4.ephec-ti.be:5000/api/auth/profile', {
        method: 'PUT',
  ...
  const handleLogout = async (e: React.MouseEvent) => {
    e.preventDefault();
    try {
      await fetch('https://m1-4.ephec-ti.be:5000/api/auth/logout', { method: 'POST', credentials: 'include' });
      setIsAuthenticated(false);
